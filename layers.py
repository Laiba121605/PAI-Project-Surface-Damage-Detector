...
class CrossEntropyLoss(nn.Module):
    """Softmax + negative log-likelihood with optional per-class weights and
    optional label smoothing.

    Label smoothing replaces the hard one-hot target with a softened
    distribution (1 - eps on the true class, eps/(C-1) on the others). It
    discourages overconfident predictions and acts as a mild regulariser -
    helpful when several classes look similar (e.g. subtle wood smudges).
    ...
    def forward(self, logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        if eps > 0.0:
            # Smoothed target distribution: (1-eps) on true class, eps/(C-1) elsewhere.
            true_lp = log_probs.gather(1, target.unsqueeze(1)).squeeze(1)  # (N,)
            mean_lp = log_probs.mean(dim=1)  # (N,) average over all classes
            # -[(1-eps)*log p_true + eps * mean(log p)]
            per_sample = -((1.0 - eps) * true_lp + eps * n_classes / max(n_classes - 1, 1) *
                            (mean_lp - true_lp / n_classes))
        else:
            per_sample = -log_probs.gather(1, target.unsqueeze(1)).squeeze(1)  # (N,)
